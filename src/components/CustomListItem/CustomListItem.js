		...
		<List
			...
			renderItem={(item, index) => (
				<List.Item style={{ backgroundColor: item.status === 'done' ? '#e6ffe6' : '#f5f5f5' }}>
					...
					<div>{index + 1} - {item.taskName}. <span><i>Deadline:</i> {moment(item.deadline).format('MMM Do YYYY')}.</span> {item.status === 'done' ? <span><i>Actual finish date:</i> {moment(item.dateFinished).format('MMM Do YYYY')}</span> : ''}</div>
